
// import { NavLink } from "react-router-dom";
// import {
//     MdDrafts,
//     MdDeleteOutline,
//     MdGridView,
// } from "react-icons/md";
// import { FaFolder } from "react-icons/fa";
// import { IoClose } from "react-icons/io5";
// import profileImg from "../assets/mailprofile.png";
// import { HiMail } from "react-icons/hi";
// import { CiStar } from "react-icons/ci";
// import { AiOutlineSend } from "react-icons/ai";

// export default function Sidebar({ closeMenu }) {
//     const navClass = ({ isActive }) =>
//         `flex items-center gap-4 px-8 py-2.5 border-l-[7px] transition-all duration-200 ${isActive
//             ? "bg-white   border-[#1FB5EB]  text-black font-bold"
//             : "border-transparent text-gray-600 hover:bg-gray-100"
//         }`;
//     return (
//         // <aside className="w-[270px] h-screen bg-[#F5F8FA] shadow-xl ">
//         <aside className="w-[270px] h-full bg-[#F5F8FA] shadow-xl overflow-y-auto">

//             <button
//                 onClick={closeMenu}
//                 className="absolute top-4 right-4 lg:hidden"
//             >
//                 <IoClose size={28} />
//             </button>

//             <div className="p-8 ">
//                 <div className="flex items-center gap-4">
//                     <img
//                         src={profileImg}
//                         alt="profile"
//                         className="w-14 h-14 rounded-full"
//                     />

//                     <div>
//                         <h3 className="text-[#23b1ee] font-semibold text-md">
//                             Mike Manson
//                         </h3>

//                         <p className="text-[#282a2b] text-sm">
//                             iamkiran@gmail.com
//                         </p>
//                     </div>
//                 </div>
//             </div>

//             <nav >



//                 <NavLink
//                     to="/"
//                     end
//                     className={({ isActive }) =>
//                         `flex items-center justify-between px-8 py-4 border-l-[7px] transition-all ${isActive
//                             ? "bg-white border-[#1FB5EB] font-bold"
//                             : "border-transparent hover:bg-gray-100"
//                         }`
//                     }
//                 >
//                     {({ isActive }) => (
//                         <>
//                             <div className="flex items-center gap-4">
//                                 <HiMail
//                                     className={`text-xl ${isActive ? "text-[#1FB5EB]" : "text-gray-500"
//                                         }`}
//                                 />
//                                 <span className="text-lg">Inbox</span>
//                             </div>

//                             <span className="bg-red-400 text-white text-sm font-light px-4 py-0.5 rounded-full">
//                                 20
//                             </span>
//                         </>
//                     )}
//                 </NavLink>


//                 <NavLink to="/flagged" className={navClass}>
//                     {({ isActive }) => (
//                         <>
//                             <CiStar
//                                 className={`text-xl ${isActive ? "text-[#1FB5EB]" : "text-gray-500"
//                                     }`}
//                             />
//                             <span className="text-lg">Flagged</span>
//                         </>
//                     )}
//                 </NavLink>

//                 <NavLink to="/draft" className={navClass}>
//                     {({ isActive }) => (
//                         <>
//                             <MdDrafts
//                                 className={`text-xl ${isActive ? "text-[#1FB5EB]" : "text-gray-500"
//                                     }`}
//                             />
//                             <span className="text-lg">Draft</span>
//                         </>
//                     )}
//                 </NavLink>

//                 <NavLink to="/sent" className={navClass}>
//                     {({ isActive }) => (
//                         <>
//                             <AiOutlineSend
//                                 className={`text-xl ${isActive ? "text-[#1FB5EB]" : "text-gray-500"
//                                     }`}
//                             />
//                             <span className="text-lg">Sent</span>
//                         </>
//                     )}
//                 </NavLink>



//                 <NavLink to="/trash" className={navClass}>
//                     {({ isActive }) => (
//                         <>
//                             <MdDeleteOutline
//                                 className={`text-xl ${isActive ? "text-[#1FB5EB]" : "text-gray-500"
//                                     }`}
//                             />
//                             <span className="text-lg">Trash</span>
//                         </>
//                     )}
//                 </NavLink>



//                 <NavLink to="/more" className={navClass}>
//                     {({ isActive }) => (
//                         <>
//                             <MdGridView
//                                 className={`text-xl ${isActive ? "text-[#1FB5EB]" : "text-gray-500"
//                                     }`}
//                             />
//                             <span className="text-lg">More</span>
//                         </>
//                     )}
//                 </NavLink>
//             </nav>

//             {/* Folders */}
//             <div className="mt-10 px-8">
//                 <div className="flex items-center justify-between mb-6">
//                     <h2 className="text-xl font-bold">
//                         Folders
//                     </h2>

//                     <button className="bg-blue-100 text-gray-600 px-3  rounded-full">
//                         Add
//                     </button>
//                 </div>

//                 <div className="space-y-5">
//                     <div className="flex items-center gap-4">
//                         <FaFolder className="text-blue-600 text-xl" />
//                         <span className="text-lg">Office</span>

//                     </div>

//                     <div className="flex items-center gap-4">
//                         <FaFolder className="text-pink-400 text-xl" />
//                         <span className="text-lg">Personal</span>
//                     </div>
//                     <div className="flex items-center gap-4">
//                         <FaFolder className="text-amber-300 text-xl" />
//                         <span className="text-lg">Freelance</span>
//                     </div>
//                     <button className="
//     w-full
//     h-[45px]
//     rounded-lg
//     bg-[#FF5F5F]
//     text-white
//     mt-10
//     mb-10
//     shadow-[0_12px_24px_rgba(255,104,104,0.35)]
// ">
//                         Compose
//                     </button>

//                 </div>
//             </div>
//         </aside>
//     );
// }


import { NavLink } from "react-router-dom";
import { MdDrafts, MdDeleteOutline, MdGridView } from "react-icons/md";
import { FaFolder } from "react-icons/fa";
import { IoClose } from "react-icons/io5";
import { HiMail } from "react-icons/hi";
import { CiStar } from "react-icons/ci";
import { AiOutlineSend } from "react-icons/ai";
import profileImg from "../assets/mailprofile.png";

const menuItems = [
    {
        path: "/",
        label: "Inbox",
        icon: HiMail,
        count: 20,
    },
    {
        path: "/flagged",
        label: "Flagged",
        icon: CiStar,
    },
    {
        path: "/draft",
        label: "Draft",
        icon: MdDrafts,
    },
    {
        path: "/sent",
        label: "Sent",
        icon: AiOutlineSend,
    },
    {
        path: "/trash",
        label: "Trash",
        icon: MdDeleteOutline,
    },
    {
        path: "/more",
        label: "More",
        icon: MdGridView,
    },
];

export default function Sidebar({ closeMenu }) {
    return (
        <aside className="w-[270px] h-full bg-[#F5F8FA] shadow-xl overflow-y-auto">
            <button
                onClick={closeMenu}
                className="absolute top-4 right-4 lg:hidden"
            >
                <IoClose size={28} />
            </button>

            {/* Profile */}
            <div className="p-8">
                <div className="flex items-center gap-4">
                    <img
                        src={profileImg}
                        alt="profile"
                        className="w-14 h-14 rounded-full"
                    />

                    <div>
                        <h3 className="text-[#23b1ee] font-semibold">
                            Mike Manson
                        </h3>

                        <p className="text-sm text-gray-700">
                            iamkiran@gmail.com
                        </p>
                    </div>
                </div>
            </div>

            {/* Menu */}
            <nav>
                {menuItems.map(({ path, label, icon: Icon, count }) => (
                    <NavLink
                        key={path}
                        to={path}
                        end={path === "/"}
                        className={({ isActive }) =>
                            `flex items-center justify-between px-8 py-4 border-l-[7px] transition-all ${isActive
                                ? "bg-white border-[#1FB5EB] font-bold"
                                : "border-transparent hover:bg-gray-100"
                            }`
                        }
                    >
                        {({ isActive }) => (
                            <>
                                <div className="flex items-center gap-4">
                                    <Icon
                                        className={`text-xl ${isActive
                                            ? "text-[#1FB5EB]"
                                            : "text-gray-500"
                                            }`}
                                    />
                                    <span>{label}</span>
                                </div>

                                {count && (
                                    <span className="bg-red-400 text-white text-sm px-4 py-0.5 rounded-full">
                                        {count}
                                    </span>
                                )}
                            </>
                        )}
                    </NavLink>
                ))}
            </nav>

            {/* Folders */}
            <div className="mt-10 px-8">
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-xl font-bold">Folders</h2>

                    <button className="bg-blue-100 text-gray-600 px-3 rounded-full">
                        Add
                    </button>
                </div>

                <div className="space-y-5">
                    {[
                        { name: "Office", color: "text-blue-600" },
                        { name: "Personal", color: "text-pink-400" },
                        { name: "Freelance", color: "text-amber-300" },
                    ].map((folder) => (
                        <div
                            key={folder.name}
                            className="flex items-center gap-4"
                        >
                            <FaFolder className={`${folder.color} text-xl`} />
                            <span>{folder.name}</span>
                        </div>
                    ))}

                    <button className="w-full h-[45px] rounded-lg bg-[#FF5F5F] text-white mt-10 mb-10 shadow-[0_12px_24px_rgba(255,104,104,0.35)]">
                        Compose
                    </button>
                </div>
            </div>
        </aside>
    );
}